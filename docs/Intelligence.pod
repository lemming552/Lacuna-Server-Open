...
=head2 recruit_spy ( session_id, building_id, [ quantity ])

Allows you to recruit more spies.

 {
    "status" : { ... },
    "recruited" : 3,
    "not_recruited" : 2,
    "reason_not_recruited" : {
        "code" : 1011,
        "message" : "Not enough food to recruit a spy.",
    },
 }
 
The only reason C<not_recruited> might be over 0 is if you specify recruiting more spies than you have resources to spend in which case the reason_not_recruited block will be included.
...
=head2 subsidize_recruiting ( session_id, building_id )

Will spend 1 essentia per spy to complete the recruiting of all spies immediately. Returns C<view>.
...
A string containing the new assignment name. Possible assignments listed below.

=head3 nexttask

A string containing the assignment to run after finishing current assignment.
Default is Idle.
...
=head2 assign_spy_nexttask ( session_id, building_id, spy_id, nexttask)

Assigns the task to perform after completing current task.
Returns same block as assign_spy
